...
def get_cur_session(login_name, login_pass):
    session = get_session()
    runntime = get_runntime('./sinalogin.js')
    su = get_encodename(login_name, runntime)
    ...
    sp = get_pass(login_pass, pre_obj, runntime)
    ...
    print("你当前使用的是resolvewang实现的微博登陆方式,登陆返回信息为:\n"+login_info)

    pattern = re.compile(r'CallBack\((.*)\);')
    rs = pattern.search(login_info)
    cont = rs.group(1) if rs.group() else ''
    uid = ''

    if cont:
        try:
            cont_obj = json.loads(cont)
            uid = cont_obj['userinfo']['uniqueid']
